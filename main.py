			...
			for hd in head_list:
				...
				if inside_box([x1r,y1r,x2r,y2r], [x1h,y1h,x2h,y2h]): # if this head inside this rider bbox
					...
					if  helmet_present[0] == True: # if helmet present
						...
						frame = cv2.putText(frame, f'{round(helmet_present[1],1)} Helmet Detected', (x1h, y1h+40), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,0,255), 1, cv2.LINE_AA)
						...
						frame = cv2.putText(frame, f'{round(helmet_present[1],1)} No Helmet', (x1h, y1h+40), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,0,255), 1, cv2.LINE_AA)
						try:
